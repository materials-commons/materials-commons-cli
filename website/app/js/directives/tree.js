...
angular.module("template/ngtree/element.html", []).run(["$templateCache", function ($templateCache) {
    var template =
        ...
        "    <img src='assets/img/rightarrowtree.png' width='16px' height='16px' style='max-width: 16px'/>" +
        "  </div>" +
        "  <div ng-show='data.children' ng-class='{show: getSelectedClass(data.id), hide: !getSelectedClass(data.id)}' class='pull-left' ng-click='toggleShow(data)'>" +
        "    <img src='assets/img/downarrow.png' width='16px' height='16px' style='max-width: 16px'/>" +
        "  </div>" +
        ...
        "        <img src=\"assets/img/checkmark.png\"  ng-class='{show: isChecked(data.id), hide: !isChecked(data.id)}'  class=\"checkBox-icon\"/>" +
        "      </span>" +
